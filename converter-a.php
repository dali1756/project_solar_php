<?php
    ...
    // 變流器編號 廠牌型號
    function get_maintain_device($db, $area_id) {
    ...
    function get_maintain($db, $area_id) {
    ...
    $area_id = 1; // 有區域變動改這 (維護廠房)
    $area_name = get_area($db, $area_id);
    $inverters = get_inverter($db, $area_id);
    $maintain_device = get_maintain_device($db, $area_id);
    $statuses = get_statuses($db, $area_id);
    $invertor_maintain = get_maintain($db, $area_id);
?>
