<?php
    ...
    // 變流器編號 廠牌型號
    function get_rolled_device($db, $area_id) {
    ...
    function get_rolled($db, $area_id) {
    ...
    $area_id = 3; // 有區域變動改這 (軋鋼西側廠房)
    $area_name = get_area($db, $area_id);
    $inverters = get_inverter($db, $area_id);
    $rolled_device = get_rolled_device($db, $area_id);
    $statuses = get_statuses($db, $area_id);
    $invertor_rolled = get_rolled($db, $area_id);
?>
                            ...
                            <tr>
                                ...
                                <?php
                                    ...
                                    foreach ($invertor_rolled as $row) {
                                        ...
                                        $ac_output_power = (($row["L1_power"] * 0.1 / 1000) + ($row["L2_power"] * 0.1 / 1000) + ($row["L3_power"] * 0.1 / 1000));
                                        if ($dc_input_power != 0) {
                                            $conversion_efficiency = number_format(($ac_output_power / $dc_input_poewr) * 100, 2);
                                        } else {
