<?php 
    ...
    if(isset($_POST['st_date'])){
        $date = $_POST['st_date'];
        $date_day   = date('Y-m-d H:i:s', strtotime($_POST['st_date']));
    }else{
        $date = null;
    }
    // 區域
    ...
    if(isset($_POST['st_date'])){
        ...
        $stmt_invertor_count->bindParam(':st_date', $_POST['st_date'], PDO::PARAM_STR);
    ...
    function Power_GenerationRows($i, $date_day_hour, $area_id, $ex_total_energy)
    {
        global $db;
    ...
    function Cumulative_SunshineRows($i, $date_day_hour, $area_id, $ex_sunshine, $ex_sunshine_add_date, $total_energy, $area_capacity)
    {
        ...
        if(count($cumulative_sunshine_list)>0){
            ...
            //$ans  = number_format(round($ans, 4) , 2);
...
<div class="content">
    <div class="container">
        ...
        <div class="row justify-content-between justify-content-md-start m-0">
          <a type="button"  href="history-total-d.php" class="btn btn-outline-primary fs-5 col-auto active">每日</a>
        ...
        <div class="col-lg-12">
              <div class="card-body">
                <!-- 查詢 -->
                <form  method="post" action="history-total-d.php">
                      <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="st_date" class="form-label">選擇日期</label>
                                <input name="st_date" type="date" class="form-control" aria-describedby="datepicker"
                                    placeholder="請選擇日期" required value="<?= $date ?>" max="<?= date('Y-m-d') ?>">
                            </div>
                        </div>
                ...
                <?php if(!empty($area) && !empty($date) && count($add_date_count)>0 ){?>
                <?php //查詢結果資料 ?>
                <div class="page-title">
                    <h3>查詢結果</h3>
                </div>
                                ...
                                <tr>
                                    ...
                                    if ($_POST['area'] == 'all') {
                                            ...
                                            for ($i = 0; $i < count($add_date_count); $i++) {
                                                //$date_day_hour = date('Y-m-d H:i:s', strtotime('+' . $i . ' hour', strtotime($date_day)));
                                            ...
                                            <tr>
                                                ...
                                                <th>
                                                    ...
                                                    for ($ii = 0; $ii < count($areas); $ii++) {
                                                        $total_energy = Power_GenerationRows($i,$date_day_hour, $ii , $ex_total_energy);
                                                ...
                                                <th>
                                                   <?php
                                                    ...
                                                    for ($ii = 0; $ii < count($areas); $ii++) {
                                                        $total_sunshine = Cumulative_SunshineRows($i, $date_day_hour, ($ii+1), $ex_sunshine, $ex_sunshine_add_date, $all_total_energy, $total_capacity);
                                    ...
                                    <?php
                                        ...
                                        if(count($add_date_count)==24){
                                            $date_day_hour = date("Y-m-d 23:59:59", strtotime($add_date_count[0]['judge_date']));
                                            $date_day_hour_show = date("Y-m-d 24:00:00", strtotime($add_date_count[0]['judge_date']));
                                    ?>
                                        <tr>
                                            <th>25</th>
                                            <th><?= $date_day_hour_show ?></th>
                                            <th>全部</th>
                                            <th><?= $total_capacity ?></th>
                                            <th>
                                            <?php
                                            ...
                                            <th>
                                                   <?php
                                                    ...
                                                    $all_total_sunshine  = number_format(round($all_total_sunshine, 4) , 2);
                                                    $all_total_sunshine = !empty($all_total_sunshine) ? str_replace(",","",$all_total_sunshine) : 0;
                                                    ?>
                                                    <?=$all_total_sunshine?>
                                            </th>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <th>
                                                <?php
                                                // 發電量(kWh)資訊
                                                $total_energy = Power_GenerationRows($i, $date_day_hour, $area, $ex_total_energy);
                                                $ex_total_energy = $total_energy[1];
                                                ?>
                                                <?=$total_energy[0]?>
                                            </th>
                                            <th>
                                                <?php
                                                // 累積日照量(kWh/m2)資訊
                                                $total_sunshine = Cumulative_SunshineRows($i, $date_day_hour, $area, $ex_sunshine, $ex_sunshine_add_date, $total_energy[0], $area_row['capacity']);
                                                $ex_sunshine = $total_sunshine[1];
                                                $ex_sunshine_add_date = $total_sunshine[2]; 
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <th>
                                                <?php
                                                // 發電量(kWh)資訊
                                                $total_energy = Power_GenerationRows(0, $date_day_hour, $area, $ex_total_energy);
                                                $ex_total_energy = $total_energy[1];
                                                ?>
                                                <?=$total_energy[0]?>
                                            </th>
                ...
                <?php  
                    }elseif(!empty($area) && !empty($date)  && empty(count($add_date_count))){
                ?>
...
<script>
    ...
    function callPHPFunction() {
            ...
            success: function(response) {
                ...
                downloadLink.download = '統計報表(每日).csv'; // 下载文件的名称
